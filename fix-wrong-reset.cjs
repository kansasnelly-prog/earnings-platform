...
async function resetDay2ForFire() {
  ...
  // Search for users with "fire" in email
  const { data: fireUsers, error: searchError } = await supabase
    .from('users')
    .select('*')
    .ilike('email', '%fire%');
  
  if (searchError) {
    console.error('Error searching for fire users:', searchError);
  } else {
    console.log('Found users with "fire" in email:');
    fireUsers.forEach(u => console.log('  -', u.email, '(ID:', u.id, ')'));
  }
