      ...
      <div className="relative z-10 w-full max-w-[1200px] mx-auto p-4 md:p-8 pb-24">
            ...
            <Card className="backdrop-blur-xl border-2 border-amber-500/30 shadow-2xl bg-gradient-to-br from-slate-900/95 to-black/95">
              <CardContent className="p-8">
                ...
                {showEmailForm && (
                  <form onSubmit={handleEmailAuth} className="mt-6 space-y-4">
                    <div>
                      ...
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" required className="w-full bg-slate-900/80 backdrop-blur-sm border-2 border-amber-500/50 rounded-lg px-4 py-3 text-amber-400 placeholder-amber-400/50 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-400/30 transition-all duration-300" />
                    </div>
                    <div>
                      <label className="block text-amber-400 font-bold mb-2 animate-glow text-sm">Password</label>
                      <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" required className="w-full bg-slate-900/80 backdrop-blur-sm border-2 border-amber-500/50 rounded-lg px-4 py-3 text-amber-400 placeholder-amber-400/50 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-400/30 transition-all duration-300" />
                    </div>
                    <Button type="submit" disabled={authLoading} className="w-full bg-gradient-to-r from-amber-500 to-orange-600 text-white font-bold py-3 rounded-xl hover:scale-105 transition-all duration-300 shadow-lg shadow-amber-500/50">
                ...
                <div className="mt-6 text-center">
                  <button type="button" onClick={() => setIsSignUpMode(!isSignUpMode)} className="text-red-400 hover:text-red-300 text-sm font-semibold transition-all duration-300 animate-glow">
                ...
                <CardContent className="p-6">
                  <div className="aspect-[3/4] bg-gradient-to-br from-pink-500/20 to-purple-500/20 rounded-xl mb-4 flex items-center justify-center">
                    {currentProfile.avatar_url ? (
                      <img src={currentProfile.avatar_url} alt={currentProfile.display_name} className="w-full h-full object-cover rounded-xl" />
                    ) : (
                  ...
                  <div className="flex gap-4">
                    <Button onClick={() => handleSwipe('left')} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl transition-all duration-300">✕ Pass</Button>
                    <Button onClick={() => handleSwipe('right')} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl transition-all duration-300">♥ Like</Button>
                  </div>
                </CardContent>
              </Card>
            ) : (
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-slate-900/95 to-black/95 backdrop-blur-xl border-t border-amber-500/30 p-4">
          <div className="max-w-[1200px] mx-auto flex items-center justify-between">
            ...
            <div className="text-amber-400/60 text-sm">© 2026 TikTok6 Network. All rights reserved.</div>
          </div>
        </div>
      </div>
    </div>
