...
const AIAssistantWorkspace = () => {
  ...
  // 🔍 ERROR DIAGNOSIS
  // File: src/pages/AIAssistantWorkspace.tsx
  // Line: 34-35
  // Error Type: Runtime (unauthenticated access)
  // Root Cause: Component renders admin UI even when no user session exists, leading to "You must be logged in" fallback elsewhere.
  // FIX METHOD: Level 2 - Small Section Fix
  // Reason: Replace the unauthenticated fallback with a login card UI.

  // If no authenticated user, show login gateway
  if (!user) {
    ...
    const handleLogin = async () => {
      ...
      try {
        ...
        // Successful login will trigger context update via useAuth
      } catch (e: any) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
        <Card className="w-full max-w-md shadow-[0_30px_70px_rgba(0,0,0,0.5)]">
          <CardHeader className="bg-gradient-to-r from-purple-600 to-blue-600 text-white">
            <CardTitle className="text-2xl font-bold">Admin Login</CardTitle>
            <CardDescription>Secure access to the 3D Admin Suite</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 pt-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
